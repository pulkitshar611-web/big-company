...
Write-Host ""
Write-Host "Seeding project data..." -ForegroundColor Cyan
npx ts-node src/seed-projects.ts

if ($LASTEXITCODE -ne 0) {
    Write-Host ""
    Write-Host "Project seeding failed!" -ForegroundColor Red
    exit 1
}

Write-Host ""
Write-Host "Database updated and project data seeded successfully!" -ForegroundColor Green
